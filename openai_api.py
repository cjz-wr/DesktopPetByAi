...
try:
    ...
    with open("demo_setting.json", "r", encoding="utf-8") as f:
        config = json.load(f)
        API_KEY = config.get("openai_key", "")  # 使用openai_key字段
        BASE_URL = config.get("openai_base_url", "https://api.openai.com/v1")  # 支持自定义base_url
        MODEL = config.get("openai_model", "gpt-3.5-turbo")  # 使用openai_model字段
...
# 注意：API_KEY、BASE_URL和MODEL现在从demo_setting.json加载
...
def load_gif():
    ...
    if found_path:
        ...
        print(f"GIF 文件夹 '{folder_path}' 不存在，正在创建该文件夹...")
        # 创建文件夹（如果不存在）
